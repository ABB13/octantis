    ...
    */
            ...
            void load_connButton(void *ptr)
            {
                ...
                while (it != wifiList.end())
                {
                    ...
                    if (stored_ssid==*it)
                    {
                        ...
                        char* st_ssid = strdup(stored_ssid.c_str());
                        char* st_pass = strdup(stored_pass.c_str());
    ...
    /* PAGE "check_conn" */////////////////////////////////////////
        // FUNCTION FOR SAVE THE WIFI PREFERENCE
            ...
            {
                if (strlen(pass) <= MAX_SIZEOF_PASS && strlen(selected_SSID) <= MAX_SIZEOF_SSID)
                {
...
*************************************************************************************************************************/
    ...
    void automatic_connection()
    {
        ...
        if (WiFi.status() == WL_CONNECTED)
        {
            ...
            // Connect to the websocket
            webSocket.begin("demo.signalk.org", 80, "/signalk/v1/stream?subscribe=none");
...
void webSocketAutomaticConnection(WStype_t type, uint8_t * payload, size_t length)
{
    switch(type) 
    {
        ...
        {
        check_connectionwebsocket = false; 
        error_status.setText("Problem with WebScoket"); 
        USE_SERIAL.printf("[WSc] Disconnected!\n");
        delay(1500);
        page_loader();
        //printStatus("Disconnected Wsc");
        ...
        check_connectionwebsocket = true;
        USE_SERIAL.println("Connected to the Wsc!");
        error_status.setText("Connected to the Wsc!");
        delay(1500);
        page_loader();
        }
        break;
        ...
        case WStype_ERROR:
        {
            check_connectionwebsocket = false; 
            USE_SERIAL.println("[WSc] ERROR");
            error_status.setText("Wsc ERROR");
            delay(1500);
            page_loader();
        }
        break;
